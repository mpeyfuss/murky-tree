...
def make_tree(
    s: str, sort_leaves: bool = True
) -> tuple[list[list[str]], StandardMerkleTree]:
    leaves = [[x] for x in s]
    tree = StandardMerkleTree.of(leaves, ["string"], sort_leaves)
    return leaves, tree
...
class TestStandartTestCase:
    ...
    def test_invalid_multiproofs(self, sort_leaves):
        ...
        assert not StandardMerkleTree.verify_multi_proof(
            tree.root, ["string"], multi_proof
        )
    ...
    def test_out_of_bonds(self, sort_leaves):
        ...
        with pytest.raises(Exception) as context:
            tree.get_proof(1)
        assert "Index out of bounds" in str(context.value)
    ...
    def test_reject_unrecognized_tree(self):
        ...
        assert "Unknown format 'nonstandard'" in str(context.value)
    ...
    def test_reject_malformed(self):
        with pytest.raises(Exception) as context:
            ...
            tree1.get_proof(0)
        assert "Merkle tree does not contain the expected value" in str(context.value)
        ...
        with pytest.raises(Exception) as context:
            ...
            tree2.get_proof(0)
        assert "Unable to prove value" in str(context.value)
    ...
    def test_render_tree(self):
        ...
        expected = (
            """0) 9c15a6a0eaeed500fd9eed4cbeab71f797cefcc67bfd46683e4d2e6ff7f06d1c"""
        )
